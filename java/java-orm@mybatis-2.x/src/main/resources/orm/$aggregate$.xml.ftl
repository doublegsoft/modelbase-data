...
<#assign typeDef = objectConstructor("com.doublegsoft.jcommons.metacode.TypeDefinition", aggregate, model)>
...
<mapper namespace="${namespace}.${app.name}.dao.${java.nameType(aggregate.name)}DataAccess">
  <sql id="join${java.nameType(aggregate.name)}">
...
<#list flow.types as typeObj>
  ...
  <sql id="column${java.nameType(aggregate.name)}">
<#list columnedAttrs as attr>
  ...
  <sql id="where${java.nameType(aggregate.name)}">
<#list columnedAttrs as attr> 
  ...
  <sql id="orderBy${java.nameType(aggregate.name)}">
  </sql>
  
  <select id="select${java.nameType(aggregate.name)}" parameterType="${namespace}.${app.name}.dto.payload.${java.nameType(aggregate.name)}Query" 
          resultType="${namespace}.${app.name}.dto.payload.${java.nameType(aggregate.name)}Query">
    select <include refid="column${java.nameType(aggregate.name)}"/>
  <#assign origObj = model.findObjectByName(flow.types[0].name)>
    ...
    <include refid="join${java.nameType(aggregate.name)}"/>
    where 1 = 1
    <include refid="where${java.nameType(aggregate.name)}"/>          
  </select>
</mapper>
