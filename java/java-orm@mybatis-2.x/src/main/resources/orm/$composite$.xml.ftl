...
<#list obj.attributes as attr>   
  ...
  <select id="select${java.nameType(obj.name)}" parameterType="${namespace}.${app.name}.dto.payload.${java.nameType(obj.name)}Query" 
          resultType="${namespace}.${app.name}.dto.payload.${java.nameType(obj.name)}Query">
    select 
<#------------->
<#-- 自身属性 -->  
<#------------->    
