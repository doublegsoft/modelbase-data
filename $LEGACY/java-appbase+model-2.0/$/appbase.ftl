...
${''?left_pad(indent)}if (${java.nameVariable(obj.name)}.get${java.nameType(attr.name)}() != null && ${java.nameVariable(obj.name)}.get${java.nameType(attr.name)}().toString() != null) {
...
<@indent_line indent=indent/>and ${modelbase.get_object_sql_alias(obj)}.${attr.persistenceName} like ${r'${_llike('}${modelbase.get_attribute_sql_name(attr)}${r'0)}'}
<@indent_line indent=indent/>${r"</#if>"}
<@indent_line indent=indent/>${r"<#if !_empty("}${modelbase.get_attribute_sql_name(attr)}${r"1)>"}
<@indent_line indent=indent/>and ${modelbase.get_object_sql_alias(obj)}.${attr.persistenceName} like ${r'${_rlike('}${modelbase.get_attribute_sql_name(attr)}${r'1)}'}
<@indent_line indent=indent/>${r"</#if>"}
...
<#macro print_sql_where_for_match obj indent>
<#assign attrIds = modelbase.get_id_attributes(obj)>
<#assign groups = modelbase.group_object_references(obj, model)>
<#assign o2mRefAttrs = groups.o2mRefAttrs![]>
<#assign o2mRefObjs = groups.o2mRefObjs![]>
...
<@indent_line indent=indent/>return '''';
  <#else>
<@indent_line indent=indent/>return ${holder}.${modelbase.get_attribute_sql_name(attr)?replace('Id','Name')} || '''';
  </#if>
</#macro>

<#macro print_object_attribute_transform_in_sql obj indent holder>
  <#list obj.attributes as attr>
    <#if attr.type.name == 'date' || attr.type.name == 'datetime'>
<@indent_line indent=indent/>if (${holder}.${modelbase.get_attribute_sql_name(attr)}) {
<@indent_line indent=indent/>  ${holder}.${modelbase.get_attribute_sql_name(attr)} = moment(${holder}.${modelbase.get_attribute_sql_name(attr)}).format(''YYYY-MM-DD'');
<@indent_line indent=indent/>}    
    <#elseif attr.constraint.domainType.name?index_of('enum') == 0>
<@indent_line indent=indent/>if (${holder}.${modelbase.get_attribute_sql_name(attr)}) {
<@indent_line indent=indent/>  ${holder}.${modelbase.get_attribute_sql_name(attr)} = ${parentApplication}.${modelbase.get_object_module(attr.parent)}.${attr.parent.name?upper_case}_${attr.name?upper_case}[${holder}.${modelbase.get_attribute_sql_name(attr)}];
<@indent_line indent=indent/>}    
    </#if>
  </#list>
</#macro>
...
<#--
 ### Transform code or date to user readable text.
 -->
<#function get_attribute_transform_in_sql holder attr>
  <#if attr.type.custom>
    <#return holder + '.' + modelbase.get_attribute_sql_name(attr)?replace('Id', 'Name')>
  <#elseif attr.type.name == 'date' || attr.type.name == 'datetime'>
    <#return holder + '.' + modelbase.get_attribute_sql_name(attr) + ' ? moment(' + holder + '.' + modelbase.get_attribute_sql_name(attr) + ').format(\'\'YYYY-MM-DD\'\') : \'\'\'\''>
  <#elseif attr.constraint.domainType.name?index_of('enum') == 0>
    <#return parentApplication + '.' + modelbase.get_object_module(attr.parent) + '.' + attr.parent.name?upper_case + '_' + attr.name?upper_case + '[' + holder + '.' + modelbase.get_attribute_sql_name(attr) + ']'>
  <#else>
    <#return holder + '.' + modelbase.get_attribute_sql_name(attr)>
  </#if>
</#function>
...
values ('CHART_WRAPPER_BY_${attrDim.name?upper_case}.${parentApplication?upper_case}.${app.name?upper_case}.${obj.name?upper_case}@DESKTOP', '0', '${obj.name?upper_case}', 'STDBIZ.SAM.MANAGED_ENTITY', 'chart${js.nameType(obj.name)}List', '【${labelObj}】（${modelbase.get_attribute_label(attrDim)}）<#if attrDim.isLabelled('when')>折线图<#else>柱状图</#if>', 'ChartWrapper', 'E', '
{
  url: ''/api/v3/common/script/${parentApplication}/${app.name}/${obj.name}/aggregate'',
  params: {
    state: ''E'',
    groupingFields: [{
      persistenceName: ''${modelbase.get_object_sql_alias(obj)}.${attrDim.persistenceName}'',
    ...
    <#if attrDim.type.custom>
      ...
      persistenceName: ''${modelbase.get_object_sql_alias(objDim)}.${modelbase.get_object_sql_alias(objDim)}nm'',
      attributeName: ''${java.nameVariable(objDim.name)}Name'',
    </#if>
    }],
    aggregateFields: [{
      operator: ''count'',
      persistenceName: ''${modelbase.get_object_sql_alias(obj)}.${attrIds[0].persistenceName}'',
    ...
    <#elseif attrDim.isLabelled('when')>
    _order_by: ''${modelbase.get_attribute_sql_name(attrDim)} asc'',
    </#if>
  },
  convert: function(row) {
    <#if attrDim.type.name == 'date' || attrDim.type.name == 'datetime'>
    row.${modelbase.get_attribute_sql_name(attrDim)} = moment(row.${modelbase.get_attribute_sql_name(attrDim)}).format(''YYYY-MM-DD'');
    </#if>
    return row;
  },
...
delete from tn_uxd_custelem where custelemid = 'CHART_WRAPPER_COUNT_BY_${attrDim.name?upper_case}.${parentApplication?upper_case}.${app.name?upper_case}.${obj.name?upper_case}@DESKTOP';
insert into tn_uxd_custelem (custelemid, parcustelemid, refid, reftyp, custelemnm, custelemtxt, custelemtyp, sta, srp)
values ('CHART_WRAPPER_COUNT_BY_${attrDim.name?upper_case}.${parentApplication?upper_case}.${app.name?upper_case}.${obj.name?upper_case}@DESKTOP', '0', '${obj.name?upper_case}', 'STDBIZ.SAM.MANAGED_ENTITY', 'chart${js.nameType(obj.name)}List', '【${labelObj}】的${modelbase.get_attribute_label(attrMetric)}（${modelbase.get_attribute_label(attrDim)}）柱状图', 'ChartWrapper', 'E', '
{
  url: ''/api/v3/common/script/${parentApplication}/${app.name}/${obj.name}/aggregate'',
  params: {
    state: ''E'',
    ...
    aggregateFields: [{
      operator: ''count'',
      persistenceName: ''distinct ${modelbase.get_object_sql_alias(obj)}.${attrMetric.persistenceName}'',
...
delete from tn_uxd_custelem where custelemid = 'CHART_WRAPPER_SUM_BY_${attrDim.name?upper_case}.${parentApplication?upper_case}.${app.name?upper_case}.${obj.name?upper_case}@DESKTOP';
insert into tn_uxd_custelem (custelemid, parcustelemid, refid, reftyp, custelemnm, custelemtxt, custelemtyp, sta, srp)
values ('CHART_WRAPPER_SUM_BY_${attrDim.name?upper_case}.${parentApplication?upper_case}.${app.name?upper_case}.${obj.name?upper_case}@DESKTOP', '0', '${obj.name?upper_case}', 'STDBIZ.SAM.MANAGED_ENTITY', 'chart${js.nameType(obj.name)}List', '【${labelObj}】的数值（${modelbase.get_attribute_label(attrDim)}）柱状图', 'ChartWrapper', 'E', '
{
  url: ''/api/v3/common/script/${parentApplication}/${app.name}/${obj.name}/aggregate'',
  params: {
    state: ''E'',
    ...
    aggregateFields: [{
      ...
      operator: ''sum'',
      persistenceName: ''${modelbase.get_object_sql_alias(obj)}.${attrNumeric.persistenceName}'',
...
<@indent_line indent=indent/><div class="full-width d-flex align-items-center text-left">
	<#if image != ''>
<@indent_line indent=indent/>  <img class="avatar size-48" src="{{${modelbase.get_attribute_sql_name(image)}}}">
	<#elseif avatar != ''>
<@indent_line indent=indent/>  <img class="avatar size-48" src="{{${modelbase.get_attribute_sql_name(avatar)}}}">
	</#if>
<@indent_line indent=indent/>  <div class="pl-2 pr-3">
	<#if primary != ''>
<@indent_line indent=indent/>    <strong>{{${modelbase.get_attribute_sql_name(primary)?replace('Id','Name')?replace('Code','Name')}}}</strong>
	</#if>
	<#if secondary != ''>    
<@indent_line indent=indent/>    <span class="small">{{${modelbase.get_attribute_sql_name(secondary)?replace('Id','Name')?replace('Code','Name')}}}</span>
	</#if>
	<#if tertiary != ''>    
<@indent_line indent=indent/>    <div class="small text-muted">{{${modelbase.get_attribute_sql_name(tertiary)?replace('Id','Name')?replace('Code','Name')}}}</div>
	</#if>
...
<@indent_line indent=indent/>  <div class="ml-auto pr-3" style="line-height: 48px;">{{${modelbase.get_attribute_sql_name(numeric)}}}</div>
	<#else>
<@indent_line indent=indent/>  <div style="margin-left: auto;">
<@indent_line indent=indent/>    <i class="fas fa-angle-right"></i>
<@indent_line indent=indent/>  </div>
	</#if>
<@indent_line indent=indent/></div>
</#macro>

<#macro print_html_cell_in_sql obj attr indent>
<@indent_line indent=indent/>title: ''${modelbase.get_attribute_label(attr)}'',
<@indent_line indent=indent/>style: ''text-align: left;'',
<@indent_line indent=indent/>display: function(row, td, rowidx, colidx) {
      <#if attr.constraint.domainType.name?index_of('enum') == 0>
<@indent_line indent=indent/>  row.${modelbase.get_attribute_sql_name(attr)}Text = ${parentApplication}.${app.name}.${obj.name?upper_case}_${attr.name?upper_case}[row.${modelbase.get_attribute_sql_name(attr)}];
      <#elseif attr.type.name == 'date' || attr.type.name == 'datetime'>
<@indent_line indent=indent/>  if (row.${modelbase.get_attribute_sql_name(attr)}) {
<@indent_line indent=indent/>    row.${modelbase.get_attribute_sql_name(attr)} = moment(row.${modelbase.get_attribute_sql_name(attr)}).format(''YYYY-MM-DD'');
<@indent_line indent=indent/>  }
      </#if>
<@indent_line indent=indent/>  let el = dom.templatize(`
<@indent_line indent=indent/>    <div>
      <#if attr.constraint.domainType.name?index_of('enum') == 0>
<@indent_line indent=indent/>      <span>{{${modelbase.get_attribute_sql_name(attr)}Text}}</span>
      <#elseif attr.type.custom>

      <#else>
<@indent_line indent=indent/>      <span>{{${modelbase.get_attribute_sql_name(attr)}}}</span>
      </#if>
<@indent_line indent=indent/>    </div>
<@indent_line indent=indent/>  `, row);
      <#if attr.type.custom>
        <#assign refObj = model.findObjectByName(attr.type.name)>
        <#assign refObjAttrId = modelbase.get_id_attributes(refObj)[0]>
        <#assign isAggregateRoot = false>
        <#list model.objects as objAggregate>
          <#if !objAggregate.isLabelled('aggregate')><#continue></#if>
          <#list objAggregate.attributes as attrAggregate>
            <#if !attrAggregate.isLabelled('root')><#continue></#if>
            <#if attrAggregate.type.name == refObj.name>
              <#assign isAggregateRoot = true>
              <#break>
            </#if>
          </#list>
...
<@indent_line indent=indent/>  let action = stdbiz.action(self.page, `
<@indent_line indent=indent/>    <a data-model-${refObj.name?replace('_', '-')}-id="{{${modelbase.get_attribute_sql_name(refObjAttrId)}}}"
<@indent_line indent=indent/>       widget-model-options=''{"columnCount":1}''
        <#if isAggregateRoot>
<@indent_line indent=indent/>       widget-model-url=":OUTLINE.${parentApplication?upper_case}.${modelbase.get_object_module(refObj)?upper_case}.${refObj.name?upper_case}@DESKTOP"
        <#else>
<@indent_line indent=indent/>       widget-model-url=":READ.${parentApplication?upper_case}.${modelbase.get_object_module(refObj)?upper_case}.${refObj.name?upper_case}@DESKTOP"
        </#if>
<@indent_line indent=indent/>       widget-model-view="sidebar"
<@indent_line indent=indent/>       class="btn-link pointer">{{${java.nameVariable(refObj.name)}Name}}</a>   
<@indent_line indent=indent/>  `, row);
<@indent_line indent=indent/>  el.appendChild(action);
      </#if>
<@indent_line indent=indent/>  td.appendChild(el);
<@indent_line indent=indent/>}
</#macro>