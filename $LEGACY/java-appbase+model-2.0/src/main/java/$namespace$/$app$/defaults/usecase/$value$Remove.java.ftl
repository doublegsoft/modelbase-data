...
public class ${java.nameType(value.name)}Remove {

  private static final Logger TRACER = LoggerFactory.getLogger(${java.nameType(value.name)}Remove.class);

  public void remove(ApplicationContext spring, ObjectMap params) throws DomainException {
    try {
