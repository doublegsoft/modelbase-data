...
public class ${java.nameType(entity.name)}Aggregate {

  private static final Logger TRACER = LoggerFactory.getLogger(${java.nameType(entity.name)}Aggregate.class);

  public List<ObjectMap> aggregate(ApplicationContext spring, ObjectMap params) throws DomainException {
  ...
  public JsonData handle(ApplicationContext spring, ObjectMap params) {
    try {
      List<ObjectMap> data = aggregate(spring, params);
