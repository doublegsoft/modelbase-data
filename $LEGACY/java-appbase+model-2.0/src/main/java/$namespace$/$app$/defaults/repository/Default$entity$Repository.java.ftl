...
public class Default${typename}Repository implements ${typename}Repository {
  ...
  private static final Logger TRACER = LoggerFactory.getLogger(Default${typename}Repository.class);
...
<#list anyRefObjs?values as refObj>
  <#if refObj.isLabelled('entity') || refObj.isLabelled('constant')>
...
<#list idAttrs as idAttr>
    ...
    // ${java.nameVariable(entity.name)}.set${java.nameType(idAttr.name)}((${modelbase.type_attribute(idAttr)}) null);
</#list>
...
<#list o2oRefAttrs as o2oRefAttr>
  ...
  <#-- o2oRefObj.name == modelbase.get_object_extension(entity) -->
  <#if o2oRefAttr.type.custom && o2oRefAttr.getLabelledOptions('extension')['role']! == 'save'>
    if (${varname}.get${java.nameType(o2oRefAttr.name)}() != null && !Strings.isBlank(${varname}.get${java.nameType(o2oRefAttr.name)}().get${java.nameType(o2oRefAttrDirectTarget.name)}())) {
    <#-- 扩展：保证主键一致 -->
...
<#list o2oRefAttrs as o2oRefAttr>
  ...
  <#if o2oRefObj.isLabelled('entity') || o2oRefObj.isLabelled('constant')>
    <#assign o2oRefObjAttrId = modelbase.get_id_attributes(o2oRefObj)[0]> 
    ${java.nameType(o2oRefObj.name)} ${java.nameVariable(o2oRefObj.name)}Ref = null;
    <#if o2oRefObj.name == entity.name>
      <#-- 说明是树结构的表 -->
      if (!id.equals(${modelbase.get_attribute_sql_name(o2oRefAttr)})) {
        // 防止数据出错，递归调用
        ${java.nameVariable(o2oRefObj.name)}Ref = ${java.nameVariable(o2oRefObj.name)}Repository.read${java.nameType(o2oRefObj.name)}(${modelbase.get_attribute_sql_name(o2oRefAttr)}, false);
        if (${java.nameVariable(o2oRefObj.name)}Ref == null) {
          ${java.nameVariable(o2oRefObj.name)}Ref = new ${java.nameType(o2oRefObj.name)}();
          ${java.nameVariable(o2oRefObj.name)}Ref.set${java.nameType(o2oRefObjAttrId.name)}(${modelbase.get_attribute_sql_name(o2oRefAttr)});
        }
      }
    <#else>
      ${java.nameVariable(o2oRefObj.name)}Ref = ${java.nameVariable(o2oRefObj.name)}Repository.read${java.nameType(o2oRefObj.name)}(${modelbase.get_attribute_sql_name(o2oRefAttr)}, false);
...
<#if entity.getLabelledOptions('persistence')['revision']??>
    try {
      commonService.execute("${entity.getLabelledOptions('persistence')['revision']}.create", params); 
    } catch (ServiceException ex) {
      TRACER.error("${entity.persistenceName?lower_case}.create", ex);
    }
</#if>
    try {
      params.set("lastModifiedTime", new Timestamp(System.currentTimeMillis()));
...
<#list o2mRefAttrs as o2mRefAttr>
  ...
  <#if o2mConjObj == ''>
    ...
    params.clear();
    try {
      params.set("${modelbase.get_attribute_sql_name(idAttrs[0])}", ${java.nameVariable(idAttrs[0].name)});
...
<#--#################################################################################################################-->
  ...
  @Override
  public List<ObjectMap> aggregate${java.nameType(entity.plural)}(ObjectMap params) throws DomainException {
    ...
    List<String> aggregate${java.nameType(entity.name)}Attributes = (List<String>) params.get("aggregate${java.nameType(entity.name)}Attributes");
    ...
    for (String aggregateAttributeName : aggregate${java.nameType(entity.name)}Attributes) {
      ObjectMap aggregateField = new ObjectMap();
      aggregateField.set("persistenceName", ${java.nameType(entity.name)}.getPersistenceName(aggregateAttributeName));
      aggregateField.set("attributeName", aggregateAttributeName);
      params.add("aggregateFields", aggregateField);
    }
    
    try {
      return commonService.all("${entity.persistenceName?lower_case}.aggregate", new SqlParams().set(params));
    } catch (ServiceException ex) {
      throw new DomainException("aggregate${java.nameType(entity.name)}", ex);
    }
  }
  <#-- 多对多关联的聚合函数 -->
  <#list entity.attributes as attr>
  ...
  @Override
  public List<ObjectMap> aggregate${java.nameType(refObj.plural)}(ObjectMap params) throws DomainException {
    ...
    List<String> aggregate${java.nameType(refObj.name)}Attributes = (List<String>) params.get("aggregate${java.nameType(refObj.name)}Attributes");
    List<String> aggregateOperatiors = (List<String>) params.get("aggregateOperators");
    ...
    int index = 0;
    for (String aggregateAttributeName : aggregate${java.nameType(refObj.name)}Attributes) {
      ObjectMap aggregateField = new ObjectMap();
      aggregateField.set("persistenceName", ${java.nameType(refObj.name)}.getPersistenceName(aggregateAttributeName));
      aggregateField.set("attributeName", aggregateAttributeName);
      aggregateField.set("operator", aggregateOperatiors.get(index++));
      params.add("aggregateFields", aggregateField);
    }
    
    try {
    <#if conjObjName != ''>
      return commonService.all("${conjObj.persistenceName?lower_case}.aggregate", new SqlParams().set(params));
    <#else>
      return commonService.all("${refObj.persistenceName?lower_case}.aggregate", new SqlParams().set(params));
    </#if>
    } catch (ServiceException ex) {
      throw new DomainException("aggregate${java.nameType(refObj.name)}", ex);
    }
  }
