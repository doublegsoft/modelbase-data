...
public class ${java.nameType(entity.name)}Sum {

  private static final Logger TRACER = LoggerFactory.getLogger(${java.nameType(entity.name)}Sum.class);
  ...
  public JsonData handle(ApplicationContext spring, ObjectMap params) {
    try {
      ...
      List<ObjectMap> data = ${java.nameVariable(entity.name)}Repository.aggregate${java.nameType(entity.plural)}(params);
