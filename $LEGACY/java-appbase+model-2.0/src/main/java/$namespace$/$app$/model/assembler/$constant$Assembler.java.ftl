...
<#list entity.attributes as attr>
  ...
  <#else>
    retVal.set${java.nameType(attr.name)}(params.get(prefixedAttributeName(prefix, "${varname}", "${modelbase.get_attribute_sql_name(attr)}")));
  </#if>
</#list>
<#list o2oRefAttrs as o2oRefAttr>
  <#assign o2oRefObj = o2oRefObjs[o2oRefAttr?index]>
...
<#list entity.attributes as attr>
  ...
  <#elseif attr.type.custom>
    ...
    ${modelbase.type_attribute_primitive(refObjIdAttr)} ${java.nameVariable(attr.name)}IdRef = params.get("${modelbase.get_attribute_sql_name(attr)}");
    if (${java.nameVariable(attr.name)}IdRef != null) {
      ${java.nameType(refObj.name)} ${java.nameVariable(attr.name)} = new ${java.nameType(refObj.name)}();
      ${java.nameVariable(attr.name)}.set${java.nameType(refObjIdAttr.name)}(${java.nameVariable(attr.name)}IdRef);
    ...
    if (${java.nameVariable(attr.name)}Object != null) {
      ${java.nameType(attr.type.name)} ${java.nameVariable(attr.type.name)} = ${java.nameType(attr.type.name)}Assembler.assemble${java.nameType(attr.type.name)}FromFrontend(${java.nameVariable(attr.name)}IdRef);
