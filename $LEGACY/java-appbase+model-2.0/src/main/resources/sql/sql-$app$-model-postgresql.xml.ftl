...
<#list model.objects as obj>
  ...
  <![CDATA[
    select <#list idAttrs as attr>${modelbase.get_object_sql_alias(obj)}.${attr.persistenceName} as ${modelbase.get_attribute_sql_name_postgresql(attr)},</#list>
  <#list selectAttrRows as row>
           <#list row as attr>${modelbase.get_object_sql_alias(attr.parent)}.${attr.persistenceName} as ${modelbase.get_attribute_sql_name_postgresql(attr)}, </#list>
  </#list>
  ...
  ${r"<#if !_empty(otherSelects)>"}
           ${r"${_keyword(otherSelects)?replace('\'\'', '\'')}"},
  ${r"</#if>"}
           0
  ...
  <#list referenceObjsAndAttrs?keys as attr>
    ...
    ${r"<#if !_empty(leftJoins)>"}
    ${r"${_keyword(leftJoins)?replace('\'\'', '\'')}"}
    ${r"</#if>"}
    where 1 = 1
<@appbase.print_sql_where obj=obj indent=4/>
  ...
  <#list obj.attributes as attr>
    ...
    ${r"<#if !_empty(andConditions)>"}
    ${r"${_keyword(andConditions)?replace('\'\'', '\'')}"}
    ${r"</#if>"}
  ...
  <sql id="${obj.persistenceName?lower_case}.revision">
  <![CDATA[
    insert into <#if databaseName??>${databaseName}.</#if>${obj.getLabelledOptions('persistence')['revision']}
