...
create table ${obj.persistenceName} (
  ...
  ${attr.persistenceName?right_pad(24)} ${r"varchar(128)"?right_pad(24)} not null<#if attr?index != attrIds?size - 1 || attrNids?size != 0>,</#if>
      <#else>
...
create table ${obj.getLabelledOptions('persistence')['revision']} (
    <#list attrIds as attr>
      <#assign domaintype = attr.constraint.domainType.name>
      <#if domaintype?index_of('&') == 0>
  ${attr.persistenceName?right_pad(24)} ${r"varchar(128)"?right_pad(24)} not null,
      <#else>
