...
<@appbase.print_sql_where obj=obj indent=4/>
  ...
  <!-- 完全匹配查询 -->
  <sql id="${obj.persistenceName?lower_case}.match">
  <![CDATA[
    select distinct 
           <#list idAttrs as attr>${modelbase.get_object_sql_alias(obj)}.${attr.persistenceName} as ${modelbase.get_attribute_sql_name_mysql(attr)},</#list>
  <#list selectAttrRows as row>
  ...
  <#list referenceObjsAndAttrs?keys as attr>
    ...
    ${r"</#if>"}
    where 1 = 1
<@appbase.print_sql_where_for_match obj=obj indent=4/>
  <#-- SPEC：多对多的关联查询 -->
  <#list obj.attributes as attr>
    <#if !attr.getLabelledOptions('persistence')['conjunction']??><#continue></#if>
    <#assign conjObj = model.findObjectByName(attr.getLabelledOptions('persistence')['conjunction'])>
  ...
  <#list attrRows as row>
    ...
    where 1 = 1
    ${r"<#if _conditions?? && !_conditions?is_string>"}
    ${r"  <#list _conditions as key, value>"}
  	${r"and ${_keyword(key)} = ${value} "}
  	${r"  </#list>"}
  	${r"<#else>"}
  	<#if (idAttrs?size == 1)>
  ...
  <#else>
    ...
    and   ${idAttr.persistenceName} = ${r'${'}${modelbase.get_attribute_sql_name(idAttr)}${r'}'}
    ${r"</#if>"}
    ${r"<#if !_empty("}${modelbase.get_attribute_sql_name(idAttr)}${r"0)>"}
		and ${idAttr.persistenceName} like ${r'${_llike('}${modelbase.get_attribute_sql_name(idAttr)}${r'0)}'}
		${r"</#if>"}
		${r"<#if !_empty("}${modelbase.get_attribute_sql_name(idAttr)}${r"1)>"}
		and ${idAttr.persistenceName} like ${r'${_rlike('}${modelbase.get_attribute_sql_name(idAttr)}${r'1)}'}
		${r"</#if>"}
    </#list>
  </#if>
    ;
...
<@appbase.print_sql_where obj=obj indent=4/>
  ...
  <![CDATA[
    ...
    where 1 = 1
    <#list idAttrs as innerAttr>
    ${r"<#if !_empty("}${modelbase.get_attribute_sql_name(innerAttr)}${r")>"}
    and ${innerAttr.persistenceName} = ${r'${'}${modelbase.get_attribute_sql_name(innerAttr)}${r'}'}
    ${r"</#if>"}
    	<#if innerAttr.type.name == 'string'>
		${r"<#if !_empty("}${modelbase.get_attribute_sql_name(innerAttr)}${r"0)>"}
		and ${innerAttr.persistenceName} like ${r'${_llike('}${modelbase.get_attribute_sql_name(innerAttr)}${r'0)}'}
		${r"</#if>"}
		${r"<#if !_empty("}${modelbase.get_attribute_sql_name(innerAttr)}${r"1)>"}
		and ${innerAttr.persistenceName} like ${r'${_rlike('}${modelbase.get_attribute_sql_name(innerAttr)}${r'1)}'}
		${r"</#if>"}
			</#if>
    </#list>
  ]]>
