...
drop table ${obj.getLabelledOptions('persistence')['revision']} if exists;
...
create table ${obj.getLabelledOptions('persistence')['revision']} (
  ...
  ${idAttr.persistenceName?right_pad(24)} ${(typebase.typename(domaintype, "sql")!'varchar(64)')?right_pad(24)} not null<#if nonIdAttrs?size != 0 || idAttr?index != idAttrs?size - 1>,</#if>
    ...
    <#list nonIdAttrs as nonIdAttr>
      ...
      <#if domaintype?index_of('&') == 0>
  ${nonIdAttr.persistenceName?right_pad(24)} varchar(64)<#if nonIdAttr?index != nonIdAttrs?size - 1>,</#if>
      <#elseif  nonIdAttr.name == 'last_modified_time'>
  ${nonIdAttr.persistenceName?right_pad(24)} ${(typebase.typename(domaintype, "sql")!domaintype)} not null<#if nonIdAttr?index != nonIdAttrs?size - 1>,</#if>
      <#else>
  ${nonIdAttr.persistenceName?right_pad(24)} ${(typebase.typename(domaintype, "sql")!domaintype)}<#if nonIdAttr?index != nonIdAttrs?size - 1>,</#if>
      </#if>
    </#list>
...
alter table ${obj.getLabelledOptions('persistence')['revision']} add constraint pk_${obj.getLabelledOptions('persistence')['revision']} primary key (<#list idAttrs as attrId>${attrId.persistenceName},</#list>lmt);
