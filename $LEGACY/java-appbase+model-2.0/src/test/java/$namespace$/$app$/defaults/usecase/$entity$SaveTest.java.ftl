 ...
 * @since ${version}
 */
public class ${java.nameType(entity.name)}SaveTest {
  ...
  public void test_usecase() throws Exception {
    System.out.println("############## ${java.nameType(entity.name)}SaveTest");
    ...
    data = (${java.nameType(entity.name)}) repositoryService.handleUsecase("${parentApplication}/${app.name}/${entity.name}/save", params);
    System.out.println(new JsonData().set("data", data));
  }

  @Test
  public void test_script() throws Exception {
    System.out.println("############## ${java.nameType(entity.name)}SaveTest -- script");
    ObjectMap params = build${java.nameType(entity.name)}("2");
    params.set("@${parentApplication}/${app.name}/${entity.name}/save", buildInner${java.nameType(entity.name)}());
    JsonData data = groovyService.execute("${parentApplication}/${app.name}/${entity.name}/save", params);
...
<#list entity.attributes as attr>
  ...
  private ObjectMap buildInner${java.nameType(entity.name)}() {
    ...
    retVal.putAll(build${java.nameType(entity.name)}("2"));
    return retVal;
  }
...
<#if idAttrs[0].type.custom>
  ...
  private ObjectMap build${java.nameType(idAttrs[0].name)}() {
    ObjectMap retVal = new ObjectMap();
<#if idAttrs[0].getLabelledOptions('extension')['role']! != 'save'>
    retVal.set("${modelbase.get_attribute_sql_name(idAttrs[0])}", "1234567890");
</#if>
