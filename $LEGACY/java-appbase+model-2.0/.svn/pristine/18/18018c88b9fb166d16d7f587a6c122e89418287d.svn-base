 ...
 * @since ${version}
 */
public class ${java.nameType(entity.name)}PaginateTest {
  ...
  public void test_script() throws Exception {
    System.out.println("############## ${java.nameType(entity.name)}PaginateTest -- script");
    ObjectMap params = build${java.nameType(entity.name)}();
    params.set("@${parentApplication}/${app.name}/${entity.name}/find", buildInner${java.nameType(entity.name)}());
    JsonData data = groovyService.execute("${parentApplication}/${app.name}/${entity.name}/paginate", params);
...
<#list entity.attributes as attr>
  ...
  private ObjectMap buildInner${java.nameType(entity.name)}() {
    ObjectMap retVal = new ObjectMap();
    retVal.set("_source_field", "${r'${'}${modelbase.get_attribute_sql_name(idAttrs[0])}${r'}'}");
    retVal.set("_target_field", "${r'${'}${modelbase.get_attribute_sql_name(idAttrs[0])}${r'}'}");
<#list entity.attributes as attr>
  <#if !attr.persistenceName??><#continue></#if>
    retVal.set("${modelbase.get_attribute_sql_name(attr)}", "${r'${'}${modelbase.get_attribute_sql_name(attr)}${r'}'}");
</#list>
    return retVal;
  }
}
