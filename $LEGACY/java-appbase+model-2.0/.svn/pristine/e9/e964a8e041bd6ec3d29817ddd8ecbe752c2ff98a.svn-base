...
<#list idAttrs as attr>
   ...
   */
  @Deprecated
  default void update${typename}(${typename} ${varname}, boolean updateChildren) throws DomainException {
    Set<String> updates = new HashSet<>();
    if (updateChildren) {
<#list entity.attributes as attr>
  <#if attr.type.collection>
      updates.add("update${java.nameType(attr.name)}");
  </#if>
</#list>
    }
    update${typename}(${varname}, updates);
  }

  void update${typename}(${typename} ${varname}, Set<String> updateChildren) throws DomainException;
...
<#list idAttrs as attr>
  ...
  List<ObjectMap> findObjectMapsBy(ObjectMap params) throws DomainException;
  <#-- 单一实体对象引用 -->
...
<#list o2mRefAttrs as o2mRefAttr>
   ...
   *        捕获到任何异常则抛出
   */
  void update${java.nameType(modelbase.get_attribute_singular(o2mRefAttr))}(${typename} ${varname}, ${java.nameType(o2mRefObj.name)} ${java.nameVariable(modelbase.get_attribute_singular(o2mRefAttr))}) throws DomainException;  
  
  List<${java.nameType(o2mRefObj.name)}> find${java.nameType(o2mRefAttr.name)}By${java.nameType(entity.name)}(${typename} ${varname}) throws DomainException;
  </#if>
...
<#list implicitReferences as implicitReferenceName, implicitReference>
  ...
  /**
   * 根据隐式引用（${implicitReferenceName}）属性查询【${label!''}】对象.
   */
  default ${typename} find${typename}By${java.nameType(implicitReferenceName)}(${modelbase.get_object_implicit_reference_as_argument(implicitReference)}) throws DomainException {
    ObjectMap params = new ObjectMap();
    <#list implicitReference as value, attr>
    params.set("${modelbase.get_attribute_sql_name(attr)}", ${modelbase.get_attribute_sql_name(attr)});
    </#list>
    List<${typename}> ${java.nameVariable(plural)} = find${java.nameType(plural)}By(params);
    if (${java.nameVariable(plural)}.size() == 0) {
      return null;
    }
    if (${java.nameVariable(plural)}.size() > 1) {
      throw new DomainException("通过${implicitReferenceName}查询到多个${label!''}】对象");
    }
    return ${java.nameVariable(plural)}.get(0);
  }
   ...
   */
  <T> T read${java.nameType(implicitReferenceName)}ById(${modelbase.get_object_implicit_reference_as_argument(implicitReference)}) throws DomainException;
  
  <T> List<T> find${java.nameType(implicitReferenceName)}sByIds(List<String> ids, String type, String plural) throws DomainException;
  </#if>
</#list>
  ...
  <#-- 创建【聚合】操作：逻辑删除                                                                                          -->
  <#--#################################################################################################################-->
  List<ObjectMap> aggregate${java.nameType(entity.plural)}(ObjectMap params) throws DomainException;
  ...
  List<ObjectMap> aggregate${java.nameType(refObj.plural)}(ObjectMap params) throws DomainException;
  </#list>
}
