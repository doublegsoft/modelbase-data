...
public class ${java.nameType(value.name)}Aggregate {

  private static final Logger TRACER = LoggerFactory.getLogger(${java.nameType(value.name)}Aggregate.class);
...
<#if !value.isLabelled('series') && entityObj??>
      ${java.nameType(entityObj.name)}Repository ${java.nameVariable(entityObj.name)}Repository = spring.getBean(${java.nameType(entityObj.name)}Repository.class);

      List<ObjectMap> data = ${java.nameVariable(entityObj.name)}Repository.aggregate${java.nameType(value.plural)}(params);
