...
Component({
  ...
  methods: {
    ...
    async render(params) {
      this.bar${js.nameType(entity.name)} = this.selectComponent('#_bar_${entity.name}');
      ...
      var option = appbase.optionEChartsBar({
        values: [{
      ...
      this.bar${js.nameType(entity.name)}.init((canvas, width, height, dpr) => {
