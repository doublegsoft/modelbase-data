...
ObjectMap read(ApplicationContext spring, ObjectMap params) {
  ...
  if (Strings.isBlank(${modelbase.get_attribute_sql_name(attrId)}) || ${modelbase.get_attribute_sql_name(attrId)} == 'null') {
  ...
  ${java.nameType(entity.name)} obj = ${java.nameVariable(entity.name)}Repository.read${java.nameType(entity.name)}(params.get("${modelbase.get_attribute_sql_name(attrId)}"), readChildren)
  if (obj == null) return new ObjectMap()

  ObjectMap ${java.nameVariable(entity.name)} = ${java.nameType(entity.name)}Assembler.assembleObjectMapToFrontend(obj)

  // 处理一对一的数据
  List<String> otherEntities = params.get("_other_entities")
  if (otherEntities != null) {
    int index = 0
    for (String otherEntity : otherEntities) {
      Object objEntity = repositoryService.readObject(${modelbase.get_attribute_sql_name(attrId)}, otherEntity)
      ${java.nameVariable(entity.name)}.set("other" + (index++), objEntity)
    }
  }
