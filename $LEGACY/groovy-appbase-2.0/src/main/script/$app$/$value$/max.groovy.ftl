...
ObjectMap max(ApplicationContext spring, ObjectMap params) {
  ...
  String fieldMax = params.get("max");

  return commonService.single("${value.persistenceName}." + fieldMax + ".max", new SqlParams().set(params))
}

ObjectMap handle(ApplicationContext spring, ObjectMap params) {
  return max(spring, params)
}
