...
ObjectMap reorder(ApplicationContext spring, ObjectMap params) {
  ...
  ObjectMap found = commonService.single("${entity.persistenceName}.find", sqlParams)
  ...
  sqlParams.set('newPosition', newPosition)
  sqlParams.set('oldPosition', oldPosition)

  if (oldPosition != null && !newPosition.equals(oldPosition)) {
    Number offset
    if (newPosition.intValue() > oldPosition.intValue()) {
      // 下降：新位置大于原位置，大于原位置且小于等于新位置的都减1
      // 3 => 7: 4,5,6,7 -1
      /*
      ...
      */
      ...
      commonService.execute('${entity.persistenceName}.descend', sqlParams);
    } else if (newPosition.intValue() < oldPosition.intValue()) {
      // 上升：新位置小于原位置，小于原位置且大于等于新位置的都加1
      // 7 => 3: 3,4,5,6 +1
      /*
      ...
      */
      ...
      commonService.execute('${entity.persistenceName}.ascend', sqlParams);
    }
    /*
    List<ObjectMap> items = commonService.many("${entity.persistenceName}.update", sqlParams)
    for (ObjectMap item : items) {
      Number position = item.get(orderKey)
      item.set(orderKey, position + offset)
      commonService.execute("${entity.persistenceName}.update", new SqlParams().set(item))
    }
    */
  }
  /*
  commonService.execute("${entity.persistenceName}.update", sqlParams)
  */
