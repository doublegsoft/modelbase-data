...
List<ObjectMap> match(ApplicationContext spring, ObjectMap params) {
  ...
  List<ObjectMap> ${java.nameVariable(modelbase.get_object_plural(entity))} = commonService.many("${entity.persistenceName}.match", new SqlParams().set(params))
...
List<ObjectMap> data = match(spring, params)
