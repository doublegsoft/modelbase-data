...
${""?left_pad(namespace?length + 11)}int          max_connections)
{
  ...
  *pdb = (${namespace}_db_p) malloc(sizeof(${namespace}_db_t));
  ...
  </#list>
{
  ...
  sqlite3_bind_text(stmt, ${attr?index + 1}, ${modelbase.get_attribute_sql_name(attr)}, -1, NULL);
    <#else>
  sqlite3_bind_text(stmt, ${attr?index + 1}, ${modelbase.get_attribute_sql_name(attr)}, -1, NULL);
    </#if>
  if (res)
...
${""?left_pad(namespace?length + obj.name?length + 15)}${typename}${""?left_pad(max_arg_len - typename?length + 2)}${attr.name}0,
${""?left_pad(namespace?length + obj.name?length + 15)}${typename}${""?left_pad(max_arg_len - typename?length + 2)}${attr.name}1<#if attr?index == attrSelect?size - 1>)<#else>,</#if>
    <#else>
${""?left_pad(namespace?length + obj.name?length + 15)}${typename}${""?left_pad(max_arg_len - typename?length + 2)}${attr.name}<#if attr?index == attrSelect?size - 1>)<#else>,</#if>
    </#if>
  </#list>
{
  ...
  <#list obj.attributes as attr>
    ...
    {
    <#if typename == 'const char*'>
      gfc_map_put(obj, "${js.nameVariable(modelbase.get_attribute_sql_name(attr))}", (void*) strdup(buff_${attr.name}));
    <#else>
  ...
  <#list refObjs as attrname, refObj>
    ...
    {
    <#if typename == 'const char*'>
      gfc_map_put(obj, "${js.nameVariable(attr.name)}${js.nameType(js.nameType(levelledAttr.name))}", (void*) strdup(buff_${attr.name}_${levelledAttr.name}));
    <#else>
      int* val = malloc(sizeof(int));
