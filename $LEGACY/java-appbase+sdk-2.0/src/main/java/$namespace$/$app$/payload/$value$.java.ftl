...
<#assign idAttrs = modelbase.get_id_attributes(value)>
<#assign valueId = idAttrs[0]>
<#assign label = modelbase.get_object_label(value)>
/**
 * ${label}数据载体封装。
 *
 ...
 */
@XmlRootElement
public class ${java.nameType(value.name)} implements Serializable {
...
<#list implicitReferences as implicitReferenceName, implicitReference>
  ...
  public String toXML() throws JAXBException {
  	...
  	classes.add(${java.nameType(value.name)}.class);
<#list implicitReferences as implicitReferenceName, implicitReference>
	<#if implicitReferenceName == 'modifier'><#continue></#if>
